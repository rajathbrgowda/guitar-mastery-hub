...
export default function Demo() {
  ...
  const completedInPhase = skills.filter(s => s.phase_index === currentPhase && s.completed).length;
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ maxWidth: 900, mx: 'auto', px: { xs: 2, sm: 4 }, py: 4 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(4, 1fr)' }, gap: 2, mb: 4 }}>
          <StatCard icon={<LocalFireDepartmentOutlinedIcon />} label="Day streak" value={`${streak} days`} />
          <StatCard icon={<AccessTimeOutlinedIcon />} label="Total hours" value={`${Math.round(totalMins / 60)}h`} />
            ...
            <CardContent>
              ...
              <List disablePadding>
                {recentSessions.map((s) => (
                  <ListItem key={s.id} disablePadding sx={{ py: 0.75, borderBottom: '1px solid', borderColor: 'divider', '&:last-child': { borderBottom: 0 } }}>
                    <ListItemText
                      primary={
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Card variant="outlined">
              <CardContent>
                ...
                <LinearProgress variant="determinate" value={phaseProgress} sx={{ height: 8, borderRadius: 4 }} />
              </CardContent>
            </Card>
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1, height: 72 }}>
                  {last7.map((day) => {
                    ...
                    const heightPct = day.duration_min > 0 ? Math.max((day.duration_min / maxMin) * 100, 12) : 4;
                    return (
                      <Box key={day.date} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                        <Box
                          sx={{
                            width: '100%',
